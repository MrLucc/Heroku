...
//    }
    ...
    @PutMapping("/update")
    public ResponseEntity<Postagem> updatePost(@Valid @RequestBody Postagem postagemParaAtualizar) {
        return ResponseEntity.status(201).body(repositorio.save(postagemParaAtualizar));
    }

    @PostMapping("/save")
    public ResponseEntity<Postagem> savePost(@Valid @RequestBody Postagem salvarPostagens) {
